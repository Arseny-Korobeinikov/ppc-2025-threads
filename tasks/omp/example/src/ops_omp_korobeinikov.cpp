...
#include "../include/ops_omp_korobeinikov.hpp"

bool korobeinikov_simpson_multidim::SimpsonTaskOmp::ValidationImpl() {
...
bool korobeinikov_simpson_multidim::SimpsonTaskOmp::PreProcessingImpl() {
...
bool korobeinikov_simpson_multidim::SimpsonTaskOmp::RunImpl() {
  double isum = 0.;
#pragma omp parallel num_threads(8)
  {
    std::vector<std::size_t> gridpos(arity_);
    std::vector<double> coordbuf(arity_);
#pragma omp for schedule(dynamic, 2000) reduction(+ : isum)
    for (int ip = 0; ip < gridcap_; ip++) {
      {
        auto p = ip;
...
bool korobeinikov_simpson_multidim::SimpsonTaskOmp::PostProcessingImpl() {

...
bool korobeinikov_simpson_multidim::SimpsonTaskStl::ValidationImpl() {
...
bool korobeinikov_simpson_multidim::SimpsonTaskStl::PreProcessingImpl() {
...
bool korobeinikov_simpson_multidim::SimpsonTaskStl::RunImpl() {  // NOLINT(readability-function-cognitive-complexity)
  ...
  for (std::size_t i = 0; i < ws; i++) {
    ...
    threads[i] = std::thread(
        [&](std::pair<std::size_t, std::size_t> r, std::promise<double>&& promise) {
          double threadsum = 0.;

          std::vector<double> coordbuf(arity_);
          for (std::size_t ip = r.first; ip < r.second; ip++) {
            auto p = ip;
        ...
        std::make_pair(iptr, iptr + leap), std::move(promise));
    iptr += leap;
  }
...
bool korobeinikov_simpson_multidim::SimpsonTaskStl::PostProcessingImpl() {

...
bool korobeinikov_simpson_multidim::SimpsonTaskAll::ValidationImpl() {
  if (world_.rank() != 0) {
...
void korobeinikov_simpson_multidim::SimpsonTaskAll::CalcSteps() {
  steps_.resize(arity_);
  std::ranges::transform(bounds_, steps_.begin(), [n = approxs_](const auto& b) { return (b.hi - b.lo) / n; });
}

bool korobeinikov_simpson_multidim::SimpsonTaskAll::PreProcessingImpl() {
  if (world_.rank() == 0) {
...
bool korobeinikov_simpson_multidim::SimpsonTaskAll::RunImpl() {  // NOLINT(readability-function-cognitive-complexity)
...
bool korobeinikov_simpson_multidim::SimpsonTaskAll::PostProcessingImpl() {
  if (world_.rank() == 0) {
